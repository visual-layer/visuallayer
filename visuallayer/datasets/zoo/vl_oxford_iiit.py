...
class VLOxfordIIITPet(Dataset):
    ...
    def report(self):
        ...
        all_issues_df = all_issues_df.loc[df['split'] == 'all'].drop('split', axis=1).reset_index(drop=True)
        
        print(f"Visual Layer Profiler found and removed the following issues from the original dataset and removed them in {self.name}\n")
